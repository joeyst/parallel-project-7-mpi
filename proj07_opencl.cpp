...
#define NUMELEMENTSW (1024)
#define BIGSIGNALFILEBIN (char*)"bigsignal.bin"
#define CSVPLOTFILE		(char*)"plot_opencl.csv"
#define NUMELEMENTS (1*1024*1024)
#define PTSIZE (NUMELEMENTS / LOCALSIZE)
#define MAXPERIODS	100
// do we want to print in csv file format?

//#define CSV

float FinalSums[MAXPERIODS];		
float Signal[NUMELEMENTS];		
float EachSums[MAXPERIODS * LOCALSIZE];			
int WorkPerProcessor = (NUMELEMENTS / LOCALSIZE);
int MaxPeriods = MAXPERIODS;

const char *		CL_FILE_NAME = { "proj07_opencl.cl" };
...
main( int argc, char *argv[ ] )
{
	...
	// DONE. Loaded into BigSignal variable. 
	...
	// We're passing in: whole signal, size of each processor's work, max periods. 
	// And array to store the sums in. 

	size_t SignalSize = NUMELEMENTS * sizeof(float);
	size_t WorkPerProcessorSize = sizeof(WorkPerProcessor);
	size_t MaxPeriodsSize = sizeof(MaxPeriods);
	size_t EachSumsSize = MAXPERIODS * LOCALSIZE * sizeof(float);

	cl_mem dSignal = clCreateBuffer( Context, CL_MEM_READ_ONLY,  SignalSize, NULL, &status );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateBuffer failed (1)\n" );

	cl_mem dWorkPerProcessor = clCreateBuffer( Context, CL_MEM_READ_ONLY,  WorkPerProcessorSize, NULL, &status );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateBuffer failed (2)\n" );

	cl_mem dMaxPeriods = clCreateBuffer( Context, CL_MEM_READ_ONLY,  MaxPeriodsSize, NULL, &status );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateBuffer failed (3)\n" );

	cl_mem dEachSums = clCreateBuffer( Context, CL_MEM_WRITE_ONLY,  EachSumsSize, NULL, &status );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueWriteBuffer( CmdQueue, dSignal, CL_FALSE, 0, SignalSize, Signal, 0, NULL, NULL );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clEnqueueWriteBuffer failed (1)\n" );

	status = clEnqueueWriteBuffer( CmdQueue, dWorkPerProcessor, CL_FALSE, 0, WorkPerProcessorSize, &WorkPerProcessor, 0, NULL, NULL );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clEnqueueWriteBuffer failed (2)\n" );

	status = clEnqueueWriteBuffer( CmdQueue, dMaxPeriods, CL_FALSE, 0, MaxPeriodsSize, &MaxPeriods, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	Kernel = clCreateKernel( Program, "DoOneLocalFourier", &status );
	if( status != CL_SUCCESS )
	...
	status = clSetKernelArg( Kernel, 0, sizeof(cl_mem), &dSignal );
	status = clSetKernelArg( Kernel, 1, sizeof(cl_mem), &dWorkPerProcessor );
	status = clSetKernelArg( Kernel, 2, sizeof(cl_mem), &dMaxPeriods );
	status = clSetKernelArg( Kernel, 3, sizeof(cl_mem), &dEachSums );
...
#ifndef CSV
	fprintf( stderr, "Number of Work Groups = %5d x %5d\n", NUMELEMENTSW/LOCALSIZE, NUMELEMENTSW/LOCALSIZE );
#endif
	...
	status = clEnqueueReadBuffer( CmdQueue, dEachSums, CL_FALSE, 0, EachSumsSize, EachSums, 0, NULL, NULL );
	if( status != CL_SUCCESS )
...
#ifdef CSV
	fprintf( stderr, "%6d , %10.2lf\n",
		LOCALSIZE*LOCALSIZE, (double)MATW*(double)MATW*(double)MATW/(time1-time0)/1000000000. );
#else
	fprintf( stderr, "Size of elements: %6d x %6d , Work Elements: %4d x %4d , GigaMultsPerSecond: %10.2lf\n",
		NUMELEMENTSW, NUMELEMENTSW, LOCALSIZE, LOCALSIZE, (double)NUMELEMENTSW*(double)NUMELEMENTSW*(double)NUMELEMENTSW/(time1-time0)/1000000000. );
#endif
	...
	clReleaseMemObject(     dSignal  );
	clReleaseMemObject(     dWorkPerProcessor  );
	clReleaseMemObject(     dMaxPeriods  );
	clReleaseMemObject(     dEachSums  );
