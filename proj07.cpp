...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		...
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			...
			// `dst` is the index of the processor 
			// `MPI_Send` is sending `PPSize` elements starting at `&BigSignal[dst * PPSize]` to the processor. 
			// We explicitly say which thread we're sending to since we're only on `BOSS`. 
	...
	else
	{
		...
		// Which thread is receiving is implied; we just receive from `BOSS`. 
		// Why PPSignal and not PPSignal[...]? 
...
DoOneLocalFourier( int me )
{
	...
	// Initializing the sums for the current thread 	
	...
	for( int p = 1; p < MAXPERIODS; p++ )
	{
		...
		{
			// me*PPSize => starting pointer 
			// time => offset within block 
